        ...
        const fetchData = async () => {
            ...
            try {
                ...
                activeCodes.forEach(code => {
                    if (code && !enabledSkuCodes.has(code)) {
                        newDiagnoses.push({ id: `asset-${code}`, type: 'asset', severity: 'critical', title: '资产映射缺失', desc: `物理层发现活跃 SKU [${code}]，但资产库未登记。`, details: { 'SKU编码': code, '建议': '前往资产名录补录' } });
                    }
                });
                ...
                if (missingSz.length > 0) newDiagnoses.push({ id: 'gap-sz', type: 'data_gap', severity: 'warning', title: '商智记录断层', desc: `本月探测到 ${missingSz.length} 个自然日销售数据缺失。`, details: { '缺失天数': missingSz.length, '最早缺失': missingSz[0] } });
                if (missingJzt.length > 0) newDiagnoses.push({ id: 'gap-jzt', type: 'data_gap', severity: 'warning', title: '广告记录断层', desc: `本月缺失 ${missingJzt.length} 天消耗明细，投产计算失准。`, details: { '缺失天数': missingJzt.length, '建议': '补录 Excel 物理表' } });
                ...
                skuGmvMap.forEach((gmv, code) => {
                    ...
                    if (roi > 8 && cost < 1000 && gmv > 3000) {
                         newDiagnoses.push({ id: `exp-${code}`, type: 'explosive', severity: 'success', title: '爆款爆发预警', desc: `SKU [${code}] 处于高投产红利期。`, details: { '当前ROI': roi.toFixed(1), '转化率': `${(cvr*100).toFixed(1)}%` } });
                    } else if (cvr > 0.12 && gmv > 500 && gmv < 2000) {
                         newDiagnoses.push({ id: `pot-${code}`, type: 'high_potential', severity: 'info', title: '高转化潜力识别', desc: `发现极高转化的 SKU，流量利用效率卓越。`, details: { '转化率': `${(cvr*100).toFixed(1)}%`, '建议': '适度提报广告计划' } });
                    }
                });
        ...
        <div className="p-6 md:p-10 w-full animate-fadeIn space-y-10">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-200 pb-8">
                <div>
                    ...
                    <p className="text-slate-500 font-medium text-xs mt-1 opacity-60">Dimensional Performance Dashboard & AI Decision Engine</p>
                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                ...
                <div className="xl:col-span-4 bg-navy rounded-[48px] p-10 text-white shadow-2xl flex flex-col relative overflow-hidden group/room">
                    ...
                    <div className="flex items-center gap-4 mb-8 relative z-10">
                        <div className="w-14 h-14 rounded-3xl bg-brand flex items-center justify-center shadow-[0_12px_30px_rgba(112,173,71,0.4)] border border-white/10 transition-transform duration-500">
                    ...
                    <div className="flex-1 relative z-10 overflow-hidden mb-8 h-[280px]">
                        {diagnoses.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center bg-white/5 rounded-[32px] border border-white/5 p-8 text-center opacity-40 italic">
                                <DatabaseZap size={48} className="text-slate-500 mb-6" />
                                <p className="text-xs font-bold text-slate-400">系统运行平稳，暂无风险</p>
                            </div>
                        ) : (
    ...
    return (
        <button onClick={onClick} className={`bg-white rounded-[40px] border text-left transition-all duration-500 group flex flex-col overflow-hidden relative active:scale-95 ${isActive ? 'ring-[6px] ring-brand/10 border-brand shadow-2xl scale-[1.02]' : 'border-slate-100 shadow-sm hover:shadow-xl hover:border-slate-200'}`}>
            ...
            <div className={`bg-slate-50/50 border-t p-5 grid grid-cols-2 gap-4 ${isActive ? 'bg-brand/5 border-brand/10' : 'border-slate-50'}`}>
                <div><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">自营店铺</p><p className="text-[11px] font-black text-slate-700 tabular-nums">{prefix}{formatVal(value.self.current)}</p></div>
                <div className="border-l border-slate-200 pl-4"><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">POP店铺</p><p className="text-[11px] font-black text-slate-700 tabular-nums">{prefix}{formatVal(value.pop.current)}</p></div>
            </div>
        </button>
    );
};
